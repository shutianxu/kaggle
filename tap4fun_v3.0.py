...
"""
'''
V3.0数据集+测试集合并，回归模型做哑变量处理 -> 78.99
'''
import pandas as pd
import numpy as np
...
features = clf_data.drop(['user_id','register_time','classification_label','prediction_pay_price'],axis=1)
...
clf_test = clf_test.drop(['user_id','register_time','classification_label','prediction_pay_price'],axis=1)
...
reg_test = choose_test.drop(['user_id','register_time','y_pre','y_pre_pro','classification_label','prediction_pay_price'],axis=1)
...
reg_features = reg_data.drop(['user_id','register_time','classification_label','prediction_pay_price'],axis=1)
